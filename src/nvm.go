...
func main() {
  ...
  switch args[1] {
	...
	case "node_mirror":
      if detail == "" {
      	fmt.Println("Current node_mirror: " + env.node_mirror)
      } else {
	...
	case "npm_mirror":
      if detail == "" {
      	fmt.Println("Current npm_mirror: " + env.npm_mirror)
      } else {
...
func CheckVersionExceedsLatest(version string) bool{
	...
	if versionOrdinal(version) <= versionOrdinal(latest){
		return false
	} else {
		return true
	}
...
// versionOrdinal
// From: https://gist.github.com/xigang/6c2f26caf32e568a03b5
